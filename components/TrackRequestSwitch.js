...
class TrackRequestSwitch extends React.Component {
  ...
  componentWillMount() {
    ...
    trackRequest = trackRequest ?
      trackRequest.settingValue : false;
    ...
    return (
      <React.Fragment>
        {this.state.trackRequest && !isFirstAvailable ?
          <React.Fragment>
            { hideDivider ? <InputDivider style={this.props.middleSectionDivider} /> : null }
            <InputSwitch
              textStyle={textStyle}
              style={style}
...
export default connect(mapStateToProps, mapActionsToProps)(TrackRequestSwitch);
