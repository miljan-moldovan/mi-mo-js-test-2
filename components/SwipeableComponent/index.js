...
function isValidSwipe(velocity, velocityThreshold, directionalOffset, directionalOffsetThreshold) {
  return Math.abs(velocity) >
    velocityThreshold && Math.abs(directionalOffset) < directionalOffsetThreshold;
}
...
export default class extends React.PureComponent {
  ...
  gestureIsClick = gestureState => Math.abs(gestureState.dx) < 5
  ...
  handleShouldSetPanResponder = (evt, gestureState) => evt.nativeEvent.touches.length === 1
