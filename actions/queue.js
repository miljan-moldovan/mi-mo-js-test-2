...
export const GET_QUEUE_ITEM_PROVIDER_STATUS = 'queue/GET_QUEUE_ITEM_PROVIDER_STATUS';
export const GET_QUEUE_ITEM_PROVIDER_STATUS_SUCCESS = 'queue/GET_QUEUE_ITEM_PROVIDER_STATUS_SUCCESS';
export const GET_QUEUE_ITEM_PROVIDER_STATUS_FAILED = 'queue/GET_QUEUE_ITEM_PROVIDER_STATUS_FAILED';
...
export const getQueueItemProviderStatus = (queueItem, callback) => (dispatch) => {
  ...
  for (let i = 0; i < queueItem.services.length; i += 1) {
    if (!queueItem.services[i].isFirstAvailable) {
      promises.push(executeAllPromises([
        Services.getServiceEmployeeCheck({
          serviceId:
          queueItem.services[i].serviceId,
          employeeId: queueItem.services[i].employee.id,
          setCancelToken: false,
        }),
        Employees.getEmployeeStatus(queueItem.services[i].employee.id, false),
  ...
  return executeAllPromises(promises).then((promises) => {
    ...
    if (servicesWithEmployeeNotStarted.length > 0) {
      ...
      const employeeNamesArray = [...new Set(servicesWithEmployeeNotStarted)]; // remove duplicates
