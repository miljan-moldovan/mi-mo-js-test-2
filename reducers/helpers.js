...
export function getEstimatedWaitTime(item) {
  ...
  const startTime = new Date().setHours(parseInt(time[1], 10) + (time[3] ? 12 : 0), parseInt(time[2], 10) || 0, 0);
...
export function getProcentCompleted(item) {
  if (item.completed === 100) {return item.completed; }

  let progressMaxTime =  moment(item.progressMaxTime, 'hh:mm:ss').minutes() + moment(item.progressMaxTime, 'hh:mm:ss').hours() * 60;
  let processTime =  moment(item.processTime, 'hh:mm:ss').minutes() + moment(item.processTime, 'hh:mm:ss').hours() * 60;
...
export function getWidthHeightDocs() {
  ...
  export function getBadgeColor(colorsId: number[]) {
    const unusedColors = colors.filter((_color) => colorsId.indexOf(_color.id) === -1);
