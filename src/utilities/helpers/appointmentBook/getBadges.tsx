...
const multiProviderUri = require('../../../../assets/svg/multi-provider-icon.svg');
...
export const getBadges = (
  ...
  const users = appointment.isMultipleProviders
    ...
    : null;
  const star = badgeData.clientHasMembership
    ? <Icon
      color="#082E66"
      size={16}
      name="star"
    ...
    />
    : null;
  const birthdayCake = badgeData.clientBirthday
    ? <Icon
      color="#082E66"
      size={16}
      name="birthdayCake"
    ...
    />
    : null;
  const checkCircle = appointment.confirmationStatus
    ? <Icon
      color="#082E66"
      size={16}
      name="checkCircle"
    ...
    />
    : null;
  const repeat = badgeData.isRecurring
    ? <Icon
      color="#082E66"
      size={16}
      name="repeatAlt"
    ...
    />
    : null;
  const badgeNL = !badgeData.clientIsNew && badgeData.clientIsNewLocally
    ? <Badge text="NL" />
    : null;
  ...
  const badgeAddons = hiddenAddonsLength > 0
    ? <Badge text={`+${hiddenAddonsLength}`} />
    : null;
  const badgeParty = badgeData.isParty
    ? <GroupBadge text={shortVersion ? '' : initials} />
    : null;
  const priority = [
    badgeNL,
    badgeN,
