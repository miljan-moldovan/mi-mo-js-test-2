...
export default class CancelAppointmentScreen extends React.Component {
  ...
  constructor (props) {
    ...
    const {currentUserEmployee: selectedProvider} = this.props;
    this.state = {
      reason: '',
      height1: 0,
      height2: 0,
  ...
  handleCancel = () => {
    ...
    if (appointments[0].isBlockTime) {
      ...
      this.props.cancelAppointment ({
        appointmentIds,
        appointmentCancellation: {reason},
      });
    }
  };
  ...
  renderCard = appointment => {
    const {isBlockTime, client, service, employee} = appointment;
    const employeeName = `${employee.name.toUpperCase ()} ${employee.lastName[0]}.`;
    const subtitle = isBlockTime
      ? employeeName
      : `${service.description.toUpperCase ()} with ${employeeName}`;
  ...
  render () {
    ...
    return (
      ...
      >
        <View style={styles.container} onLayout={this.measureView}>
          <ScrollView
          ...
          <View style={styles.bottomContainer} onLayout={this.measureView2}>
            ...
            <Text style={styles.footer}>
              IN ORDER TO CANCEL THIS APPOINTMENT, PLEASE ENTER
