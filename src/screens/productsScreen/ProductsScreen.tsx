...
class ProductsScreen extends React.Component {
  static navigationOptions = ({navigation}) => {
    ...
    const title = selectedCategory
      ? get (selectedCategory, 'name', 'Products')
      : 'Products';
    const leftButton = selectedCategory
      ? <Icon name="angleLeft" color={Colors.white} size={24} />
      : <Text style={styles.headerButton}>Cancel</Text>;
    const leftButtonOnPress = selectedCategory
      ? clearCategory
      : navigation.goBack;
    return {
      header: props => (
  ...
  shouldComponentUpdate (nextProps) {
    return (
      ...
      nextProps.searchHeaderState.showFilter !==
        this.props.searchHeaderState.showFilter ||
      nextProps.searchHeaderState.searchText !==
        this.props.searchHeaderState.searchText ||
  ...
  onChangeProduct = item => {
    ...
    if (isFunction (onChangeWithNavigation)) {
      onChangeWithNavigation (item, navigation);
    } else if (isFunction (onChangeProduct)) {
      onChangeProduct (item);
      if (dismissOnSelect) {
        navigation.goBack ();
      }
  ...
  get currentData () {
    ...
    const selectedCategory = categories.find (
      itm => isNumber (categoryId) && get (itm, 'id', null) === categoryId
    );
    if (searchText.length > 0 || showFilter) {
      const filterFunc = itm =>
        searchText === '' ||
        get (itm, 'name', '')
          .toLowerCase ()
  ...
  scrollToLetter = letter => {
    for (let i = 0; i <= this.currentData.length; i += 1) {
      const firstChar = get (this.currentData[i], 'name', '')
        .toUpperCase ()
        .charAt (0);
  ...
  renderCategoryItem = ({item}) => {
    ...
    const productCategory = selectedProduct
      ? filter (this.currentData, {
          inventoryItems: [{id: selectedProduct.id}],
        })[0]
      : {};
    ...
    const highlightStyle = checked
      ? [styles.itemText, styles.container, {color: Colors.selectedGreen}]
      : [styles.itemText, styles.container];
  ...
  renderProductItem = ({item}) => {
    ...
    const {searchHeaderState: {searchText}} = this.props;
    const isSelected =
      isNumber (get (selectedProduct, 'id', null)) &&
      isNumber (get (item, 'id', null)) &&
    ...
    return (
      <SalonTouchableOpacity onPress={onPress} style={styles.itemRow}>
  ...
  render () {
    const {productsState: {isLoading}} = this.props;
    return (
